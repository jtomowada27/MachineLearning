...
Original file is located at
    https://colab.research.google.com/drive/16pujK8OcCRM3AgXUFMXMUWe0qzcPGcFa
"""
...
train_y = torch.tensor(train_y, dtype=torch.float).squeeze(1)
test_x = torch.tensor(test_x, dtype=torch.float).unsqueeze(1)
test_y = torch.tensor(test_y, dtype=torch.float).squeeze(1)
...
with torch.no_grad():
    for data in test_loader:
        ...
        ground_truth += targets.cpu().numpy().reshape(-1,1).tolist()
        out = rnn(inputs).detach().cpu().numpy().reshape(-1,1).tolist()
        prediction += out
...
r2score = r2_score(np.array(ground_truth), np.array(prediction))
